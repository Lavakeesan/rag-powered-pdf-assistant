...
export default function ProfilePage() {
  ...
  const [user, setUser] = useState({ fullName: 'Loading...', email: 'Loading...', role: 'Student' });
  ...
  const handleSave = () => {
    ...
    const updatedUser = { ...user, fullName: fullNameInput };
    setUser(updatedUser);
    ...
    setTimeout(() => {
      setIsSaving(false);
      // Hard refresh header or update system
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="glass-card p-8 rounded-[40px] border border-white/5 text-center flex flex-col items-center h-fit">
          <div className="relative group mb-6">
            <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-3xl font-bold text-white shadow-2xl">
          ...
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-6">{user.role} Plan</p>
        </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-950/50 border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-sm focus:outline-none focus:border-indigo-500 transition-all text-white" 
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  type="email" 
                  value={user.email} 
                  className="w-full bg-slate-950/50 border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-sm focus:outline-none opacity-70 cursor-not-allowed text-slate-400" 
                  disabled 
                />
              </div>
            </div>
