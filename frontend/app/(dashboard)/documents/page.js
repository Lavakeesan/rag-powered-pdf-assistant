...
export default function DocumentsPage() {
  ...
  const fetchDocuments = async (email) => {
    ...
    try {
      ...
      setError(
        'Could not connect to the server or load S3 bucket. Ensure your backend is running.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDeleteDoc = async (key, filename) => {
    if (
      !confirm(`Are you sure you want to permanently delete "${filename}" from S3 storage?`)
    )
      return;
  ...
  // 3. View document handler – opens PDF via pre-signed URL in new tab
  const handleViewDoc = async (key) => {
    // Open the window BEFORE the async call so browsers don't block it
    ...
    try {
      ...
      if (data && data.url) {
        ...
        newTab.close();
        alert('Unable to open PDF: invalid response');
      }
    } catch (err) {
      newTab.close();
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-slate-400 font-semibold tracking-wider text-sm">
            LOADING YOUR S3 WORKSPACE...
          </p>
        </div>
      ) : error ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredDocs.map((doc) => (
            ...
            >
              ...
              <h3
                className="text-lg font-bold text-white mb-2 truncate group-hover:text-cyan-400 transition-colors"
                title={doc.name}
              >
                {doc.name}
              </h3>
                ...
                {filteredDocs.map((doc) => (
                  <tr key={doc.id} className="hover:bg-white/5 transition-colors group">
                    <td className="p-6 font-semibold text-white flex items-center gap-3">
                      ...
                      <span className="truncate max-w-xs md:max-w-md" title={doc.name}>
                        {doc.name}
                      </span>
                    </td>
                    <td className="p-6 text-slate-400 font-medium">{doc.date}</td>
                    <td className="p-6 text-slate-400 font-medium uppercase tracking-wider">
                      {doc.size}
                    </td>
                    <td className="p-6">
